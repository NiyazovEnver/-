...
$articles_data = mysql_query($sql_q);
		...
		<div id="wrapper_right_block">
			<article> 
				...
				<?php 
					...
					while ($row = mysql_fetch_assoc($articles_data)) {
						echo "<section>
							<h2>{$row['title']}</h2>
							{$row['content']}
							<p>{$row['date']}</p>
