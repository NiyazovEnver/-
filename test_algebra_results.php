			...
			<article style="text-align: center; padding: 30px;"> 
				<?php
					...
					if (empty($_SESSION['login']) or empty($_SESSION['id'])) {
                        ...
                        $cur_id = $_SESSION['id'];
                        $user_row = mysql_query("SELECT * FROM users WHERE id = '$cur_id'"); # ищем строку с нашим пользователем
                        
                        # ЕСЛИ ПОЛЬЗОВАТЕЛЬ ТОЛЬКО ЧТО ПРОШЁЛ ТЕСТ
                        if ($_GET['score']) {
                            $user_score = $_GET['score'];
                            # вносим оценку в строку с нашим пользователем
                            $result = mysql_query("UPDATE users SET test_score='$user_score' WHERE id='$cur_id'");
